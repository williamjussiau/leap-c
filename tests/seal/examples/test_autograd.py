...
from seal.mpc import Parameter
...
def test_MPCSolutionModule_on_LinearSystemMPC(generate_code: bool = False,
    ...
    mpc = set_up_mpc(generate_code, build_code, json_file_prefix, learnable_params=["A", "B", "b", "f", "V_0"])
    
    test_param = set_up_test_parameters(mpc, batch_size, varying_param_label=varying_param_label).T  # (batch_size, 12)
    
    p_rests = [Parameter(None, None, None, None) for i in range(batch_size)]
    ...
    torch.autograd.gradcheck(fix_p_rests, (x0, p), atol=1e-2, eps=1e-4, raise_exception=True)


def test_CleanseAndReduce():
    cleansed_loss = CleanseAndReducePerSampleLoss(
        reduction="mean", num_batch_dimensions=1, n_nonconvergences_allowed=2, throw_exception_if_exceeded=False)
...
def test_CleanseAndReduceMultipleBatchAndSampleDims():
    cleansed_loss = CleanseAndReducePerSampleLoss(
        reduction="mean", num_batch_dimensions=2, n_nonconvergences_allowed=4, throw_exception_if_exceeded=False)
    ...
    status[2, 2] = 2
    cleansed_loss = CleanseAndReducePerSampleLoss(
        reduction="mean", num_batch_dimensions=2, n_nonconvergences_allowed=4, throw_exception_if_exceeded=True)
    try:
        loss = cleansed_loss(x, status)
        assert False
    except ValueError:
